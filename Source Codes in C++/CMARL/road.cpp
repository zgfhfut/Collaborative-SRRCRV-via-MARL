...
// Constructor initialization, can also be written as:
...
void Graph::printGraph(ofstream& outfile)
{
	for (int i = 0; i < N; ++i)
	{
		...
		// 'iter' is a pointer pointing to the address of each element in adj[], the type is complex, so 'auto' is convenient
...
void Graph::shortestDis(int src, vector<double>& dis_store)
{
	...
	set< pair<double, int> > setds; // Each element in setds is a pair: 1. double: the shortest distance from the corresponding node to the source 'src'; 2. int: the corresponding node ID
	...
	setds.insert(make_pair(0, src)); // Set the distance from src to itself as 0, meaning src is the starting point
	dis[src] = 0;
	...
	while (!setds.empty())
	{
		...
		for (list<Edge>::iterator iter = adj[u].begin(); iter != adj[u].end(); ++iter)
		{
			...
			if (dis[v] > dis[u] + iter->weight)
			{
				...
				so we first remove the original (dis[v], v) from setds, then update with the new shortest distance.
				*/
				if (dis[v] != INF)
...
void Graph::shortestPath(int src, vector<double>& dis_store, vector<vector<int>>& all_path_store)
{
		...
		for (list<Edge>::iterator iter = adj[u].begin(); iter != adj[u].end(); ++iter)
		{
			...
			if (dis[v] > dis[u] + iter->weight)
			{
				...
				*/
				...
				// The path can be reconstructed: for example, path[1]=5 means the previous node of node 1 in the path is node 5.
			}
		}
	}
	...
	for (int i = 0; i < N; ++i)
	{
		...
		// all_path_store stores the shortest paths from all nodes to src
...
void Graph::shortestPath1(int src, vector<double>& dis_store, vector<vector<int>>& all_path_store)
{
	...
	for (int i = 0; i < N; ++i)
	{
		...
		// all_path_store stores the paths from all nodes to src
		all_path_store.push_back(temp_path_store); // Store the shortest path from node i to src into the 2D array all_path_store
	}
}
...
void Graph::storePath(int dest, vector<int>& path, vector<int>& temp_path_store)
{
	if (path[dest] != INF)
	{
		...
		// These segments together form the shortest distance path from (dest to src)
	}
	else
	{
