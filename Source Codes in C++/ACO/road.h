...
struct Line
{
	...
	bool operator< (const Line& line) const; // Custom comparison operator <, because it will be used in map(<Line>,double)
	// map is an ordered associative container, meaning it sorts the keys (<Line>), which is equivalent to defining a custom sorting rule
