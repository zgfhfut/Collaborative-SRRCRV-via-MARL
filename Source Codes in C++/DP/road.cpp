...
// set will choose insertion position based on element size, but Line is a custom class and cannot be compared by default, so we need to define a custom comparison standard for Line.
...
void Graph::addEdge(int u, int v, double w)
{
    ...
    adj[v].push_back(Edge(v, u, w)); // Record this edge at both the starting and ending nodes
}
...
void Graph::printGraph(ofstream& outfile)
{
    for (int i = 0; i < N; ++i)
    {
        ...
        // 'auto' is a type specifier that can infer the variable type from its initial value.
        // 'iter' is a pointer pointing to elements in adj[], and it's inconvenient to write its type, so 'auto' can be used.
...
void Graph::shortestDis(int src, vector<double>& dis_store)
{
    ...
    // Each element in setds is a pair: 1. double: shortest distance from the corresponding node to the source 'src'; 2. int: the corresponding node ID.
    set< pair<double, int> > setds;

    // Create a vector 'dis' of size N with default value INF, to store the shortest distance from each node to the source.
    vector<double> dis(N, INF);

    setds.insert(make_pair(0, src)); // Set the distance from src to itself as 0, making src the starting point.
    dis[src] = 0;

    // Loop until setds is empty.
    while (!setds.empty())
    {
        pair<double, int> tmp = *(setds.begin()); // Store the first element of setds in a temporary variable, then remove it from setds.
        setds.erase(setds.begin()); // Each iteration of the while loop removes the first element of setds.

        int u = tmp.second; // Extract the node ID from the first element.

        for (list<Edge>::iterator iter = adj[u].begin(); iter != adj[u].end(); ++iter)
        {
            int v = iter->dest; // Extract the ID of the node adjacent to u.

            if (dis[v] > dis[u] + iter->weight)
            {
                /*
                Purpose of the conditional statement:
                If the stored distance from src to v (dis[v]) is not infinity (INF), it means a path 'path1' connecting v was found in previous traversals,
                and path1's distance was stored in setds as the shortest path distance. But now a shorter distance is found (dis[v] > dis[u] + iter->weight),
                so we first remove the old (dis[v], v) from setds, then update with the new shortest distance.
                */
                if (dis[v] != INF)
                    setds.erase(setds.find(make_pair(dis[v], v)));
                // Note: The node removed from setds must have a defined finite distance. Otherwise, that node could never be reached.
...
// Find the shortest distance to point src [considering the case where damaged paths can be used => travel cost = path length / v (v>1)]
void Graph::shortestDis_EX(int src, vector<double>& dis_store)
{
    ...
    while (!setds.empty())
    {
        ...
        for (list<Edge>::iterator iter = adj[u].begin(); iter != adj[u].end(); ++iter)
        {
            ...
            // Consider the case of using damaged nodes, calculate the cost of passing through the road.
...
// Find the shortest path to point src [considering the case where damaged paths can be used => travel cost = path length / v (v>1)]
